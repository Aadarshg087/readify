...
const Login = () => {
  ...
  async function handleFormData(formData) {
    ...
    try {
      ...
      const entry = await axios.post(`${BASE_URL}/loginUser`, formData);

      if (entry.status >= 200 && entry.status < 300) {
        const data = {
          name: entry.data.LoggedInUser.fullName,
          email: entry.data.LoggedInUser.email,
        ...
        setServerError(entry.data.message);
      }
    } catch (error) {
